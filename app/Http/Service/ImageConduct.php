...
//        return Response::make($content)->header('Content-Type', 'image/png');
    ...
    public function getThinkPic()
    {
        ...
        $editor->text($bgImage, $weather->monthUpper[$date['mon']], 16, 253, 75, new Color('#000000'),
            base_path('Image') . '/ttf/wryh.ttf', 0);
        $editor->text($bgImage, $date['mday'], 28, 260, 100, new Color('#000000'), base_path('Image') . '/ttf/wryh.ttf',
            0);
        $editor->draw($bgImage,
            Grafika::createDrawingObject('Line', array(250, 135), array(295, 135), 14, new Color('#000000')));//添加横线
        $editor->text($bgImage, $date['weekday'], 16, 235, 140, new Color('#000000'),
            base_path('Image') . '/ttf/wryh.ttf', 0);

        //添加文字
        $editor->text($bgImage, '//写不完的功能', 16, 200, 780, new Color('#000000'), base_path('Image') . '/ttf/wryh.ttf',
            0);
        $editor->text($bgImage, '//改不完的BUG', 16, 200, 810, new Color('#000000'), base_path('Image') . '/ttf/wryh.ttf',
            0);
     ...
     * @author 王玉翔
     */
    public function getWeatherAndLocation(
        $weatherImgNum,
        $weatherString,
        $locationString,
        $color = '#FFFFFF',
        ...
        $this->editor->text($wl, $weatherString, 13, 55, 45, new Color($color),
            base_path('Image') . '/ttf/wryh.ttf', 0);
        $this->editor->text($wl, $locationString, 13, $width - 20 - mb_strlen($locationString) * 14, 40,
            new Color($color),
            base_path('Image') . '/ttf/wryh.ttf', 0);
        return $wl;
    }
